...
class MainWindow(QWidget):
    ...
    def setupUi(self):
        self.setWindowTitle("공정 데이터 LLM 분석기 V2.2 (PyVista)")
    ...
    def setup_viz_tab(self):
        ...
        sim_layout.addWidget(self.btn_toggle_playback); sim_layout.addWidget(self.btn_reset_simulation); sim_layout.addSpacing(20)
        sim_layout.addWidget(speed_label); sim_layout.addWidget(self.speed_slider); sim_layout.addWidget(self.speed_value_label)
        
        control_layout.addWidget(self.sim_controls_widget)
        control_layout.addWidget(self.btn_ask_llm_about_viz)
        self.sim_controls_widget.hide() # 기본적으로 숨김
        
        self.viz_stack = QStackedWidget()
        self.mpl_widget = QWidget(); mpl_layout = QVBoxLayout(self.mpl_widget)
        ...
        ax = self.viz_fig.add_subplot(111); ax.text(0.5, 0.5, "Please load a CSV file...", ha='center'); ax.axis('off')
        
        layout.addWidget(control_panel); layout.addWidget(self.viz_stack, 1)
    ...
    def run_stability_analysis(self):
        self._run_visualization('plot_stability', "stability analysis graph.")
    def run_correlation_dashboard(self):
        self._run_visualization('plot_correlation_dashboard', "correlation analysis dashboard.")
    def run_3d_path_analysis(self):
        self._run_visualization('plot_3d_path', "static 3D process path visualization.")
    ...
    def run_process_simulation(self):
        ...
        if not self.visualizer: return QMessageBox.warning(self, "Warning", "Please load a CSV file first.")
        try:
            (self.sim_data, self.sim_points, self.sim_head_actor, self.sim_bead_actor) = \
                self.visualizer.prepare_pyvista_simulation(self.pv_plotter)
            if self.sim_data is None or self.sim_data.empty:
                QMessageBox.information(self, "Info", "No data for simulation (check LASER_ON status or time format).")
                self.viz_stack.setCurrentIndex(0); self.sim_controls_widget.hide()
                return
            self.reset_simulation() # 초기화 함수 호출로 준비 완료
    ...
    def reset_simulation(self):
        ...
        if self.sim_data is not None:
            self._update_simulation_frame() # 첫 프레임으로 화면을 그림
            self.sim_frame_index = 0 # 인덱스 다시 0으로 (업데이트 함수에서 1 증가하므로)
    ...
    def _update_simulation_frame(self):
        ...
        current_point = self.sim_points[self.sim_frame_index]
        self.sim_head_actor.SetPosition(current_point)
        
        if self.sim_frame_index > 0:
            if self.sim_frame_index % 5 == 0 or self.sim_frame_index == len(self.sim_data) - 1:
                line = pv.PolyData(self.sim_points[:self.sim_frame_index + 1])
                new_bead_mesh = line.tube(radius=0.3)
                self.sim_bead_actor.copy_from(new_bead_mesh)
        ...
        if self.sim_frame_index + 1 < len(self.sim_data):
            ...
            self._stop_simulation_if_running() # 마지막 프레임이면 정지
